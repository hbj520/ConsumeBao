...
@interface StoreImgViewController ()<UIActionSheetDelegate,UIPickerViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,CTAssetsPickerControllerDelegate,UITextFieldDelegate,uploadImageDelegate,UIAlertViewDelegate>
{
...
- (void)viewDidLoad {
    ...
    firstImg.image=[UIImage imageNamed:@"1.jpg"];
...
//    [self.view addSubview:titilLabel];
    ...
    detailLabel.text=@"建议上传图片尺寸在2000*1500以上,与店铺经营实体符合，要求无水印";
    ...
    _uploadBtn.frame=CGRectMake(W(20), kDown(_coverView)+H(180), WIDTH-W(20)*2, 40);
    _uploadBtn.backgroundColor=RGBACOLOR(277, 97, 0, 1);
...
- (void)assetsPickerController:(CTAssetsPickerController *)picker didFinishPickingAssets:(NSArray *)assets
{
    ...
    for (int i=0; i<_assets.count; i++) {
        ...
        _view=[[UIView alloc]initWithFrame:CGRectMake(W(10)+(i%5)*W(60), (i/5)*H(60)+H(10), W(40), H(40))];
        //        view.backgroundColor=[UIColor cyanColor];
        _view.tag=i+1;
        [_coverView addSubview:_view];
        
        UIImageView *img=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, W(40), H(40))];
        img.contentMode=3;
    ...
    _addButton.frame=CGRectMake(_assets.count*(W(60))+W(10), H(10), W(40), H(40));
...
//        [self start:view];
    UIAlertView * alert=[[UIAlertView alloc]initWithTitle:@"温馨提示" message:@"是否删除所选图片" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    alert.tag=view.tag;
    [alert show];
//    }
...
//            [self stop:view];
            ...
            int i=0;
            UIView * subViews=(UIView *)[_coverView viewWithTag:alertView.tag];
            [subViews removeFromSuperview];
            [_assets removeObjectAtIndex:alertView.tag-1];
            [_imgArr removeObjectAtIndex:alertView.tag-1];
            ...
            for (UIView * view in [_coverView subviews]) {
                ...
                {
                    view.frame=CGRectMake(W(10)+i*W(60), H(10), W(40), H(40));
                    view.tag=i+1;
                    i++;
                }
...
#pragma mark - 上传按钮绑定的方法
-(void)uploadImg
{
    NSDictionary * dict=@{@"imgArr":_sotreImgUrl};
    _block(dict);
    DMLog(@"上传图片");
