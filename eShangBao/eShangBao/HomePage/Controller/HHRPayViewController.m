...
@interface HHRPayViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    int _choose;
    int _payStyle;
...
@implementation HHRPayViewController
...
-(void)loadUI
{
    if (!_scrollerView) {
        ...
        _scrollerView.contentSize=CGSizeMake(WIDTH, H(75)+H(10)+H(60)+H(10)+H(60)*3+140);
        _scrollerView.showsHorizontalScrollIndicator = NO;
        _scrollerView.showsVerticalScrollIndicator = NO;
        [self.view addSubview:_scrollerView];
        
        DMLog(@"____%@,%@,%@",_nameArr,_priceArr,_levelArr);
        UIImageView * adImg=[[UIImageView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, H(75))];
        adImg.image=[UIImage imageNamed:@"zhb_bg.jpg"];
        adImg.contentMode=2;
        [_scrollerView addSubview:adImg];
        
        _tableView=[[UITableView alloc]initWithFrame:CGRectMake(0, kDown(adImg)+H(10), WIDTH, H(50)) style:0];
...
-(UITableViewCell * )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.titleLabel.text=_projectModel.name;
    cell.detailTitleLabel.text=[NSString stringWithFormat:@"支付金额: ￥%@",_projectModel.price];
    ...
    else
    {
        if (_payStyle==0) {
            ...
            DMLog(@"---%@,%@,%d,%@",_name,_idcard,_payStyle,_inviteCode);
                NSDictionary * param=@{@"name":_name,@"idcardno":_idcard,@"partnerLevel":_projectModel.levelId,@"payMethod":[NSString stringWithFormat:@"%d",_payStyle],@"inviteCode":_inviteCode,@"payType":[NSString stringWithFormat:@"%d",_payStyle]};
...
//            else
//            {
//                NSDictionary * param=@{@"name":_tuiName,@"idcardno":_idNumber,@"partnerLevel":_partnerLevel,@"payMethod":[NSString stringWithFormat:@"%d",_payStyle],@"type":_etype,@"payType":[NSString stringWithFormat:@"%d",_payStyle],@"phone":_phone,@"captcha":@"2"};
...
//            }
        ...
        else
        {
            // 首页进入
//            if (_whichPage==0) {
                NSDictionary * param=@{@"name":_name,@"idcardno":_idcard,@"partnerLevel":_projectModel.levelId,@"payMethod":@"0",@"inviteCode":_inviteCode,@"payType":[NSString stringWithFormat:@"%d",_payStyle]};
                [RequstEngine requestHttp:@"1024" paramDic:param blockObject:^(NSDictionary *dic) {
                    DMLog(@"1024--%@",dic);
                    DMLog(@"error---%@",dic[@"errorMsg"]);
                    if ([dic[@"errorCode"] intValue]==00000) {
...
//                        order.amount =_projectModel.price;  //商 品价格
//#warning 此处需要主要正式版本
...
//            {
//                // 推荐合伙人
//                NSDictionary * param=@{@"name":_tuiName,@"idcardno":_idNumber,@"partnerLevel":_partnerLevel,@"payMethod":@"0",@"type":_etype,@"payType":[NSString stringWithFormat:@"%d",_payStyle],@"phone":_phone,@"captcha":@"2"};
